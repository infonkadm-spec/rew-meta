...
export default function HotmartCheckout({
  ...
  variant,
  email,
}: HotmartCheckoutProps) {
  const isWhiteContent = variant === "white";
  ...
  const initCheckout = () => {
    ...
    // @ts-expect-error
      ...
      // COMMENTED FOR A/B TESTING - UNCOMMENT TO ENABLE PRODUCT HIDING
      if (variant === "black") {
          ...
          iframes.forEach(iframe => {
            if (iframe instanceof HTMLIFrameElement) {
              ...
              iframeOverlay.innerHTML = `
                <div class="flex items-start gap-5 mx-7 mt-7 mb-1" style="position: relative; z-index: 99999; display: flex; visibility: visible; opacity: 1; pointer-events: auto;">
                  <div class="flex-none relative">
                    <img alt="Logo" width="200" height="137" class="w-20 aspect-square rounded-full" src="/images/logo.png">
                    <img alt="Logo" width="20" height="20" class="absolute bottom-0.5 right-0.5" src="/images/instagram-verify.png">
                  </div>
  ...
  return (
    <div className="flex flex-col flex-auto rounded-3xl shadow-xl relative overflow-hidden border shadow-black/20 border-gray-100">
      {/* COMMENTED FOR A/B TESTING - UNCOMMENT TO ENABLE PRODUCT HIDING CSS */}
      {variant === "black" && (
        <style jsx global>{`
      ...
      {/* COMMENTED FOR A/B TESTING - Product element will be cloned and positioned over iframe */}
      {false && (
        <div className="flex items-center gap-5 mx-7 py-6" style={{ minHeight: '120px' }}>
          <div className="flex-none relative">
            <Image
              src={isWhiteContent ? "/images/kimplan.jpg" : "/logo.svg"}
              alt="Logo"
              width={100}
              height={100}
              className="w-20 h-20"
            ...
            />
            <Image
              src="/images/instagram-verify.png"
              alt="Logo"
              width={20}
              height={20}
          ...
          <div className="flex flex-col gap-1">
            {isWhiteContent && <span className="text-sm font-semibold text-balance italic -mt-0.5 -mb-1.5 opacity-30">Kim Kardashian&apos;s</span>}
            <span className="text-base font-semibold text-balance tracking-tighter">YouTube Rewards</span>
            <span className="text-xs line-through tracking-wider leading-none text-gray-400">From US$ 129,90 for</span>
            <div className="flex items-center gap-1 -mt-0.5">
              <span className="text-sm leading-none mt-1 text-green-600">Only</span>
              <span className="text-2xl font-semibold tracking-tighter leading-none text-green-600">US$ {priceList[offer || "wbz5t6cu"]}</span>
            </div>
          </div>
        </div>
      )}
